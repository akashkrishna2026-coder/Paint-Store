...
class _TexturesPageState extends State<TexturesPage> {
  ...
  Future<List<Map<String, dynamic>>> _fetchTextures() async {
    ...
    final snapshot = await _texturesRef.orderByChild('category').equalTo(widget.category).get();
    List<Map<String, dynamic>> textures = [];

    if (snapshot.exists && snapshot.value != null) {
      final data = Map<String, dynamic>.from(snapshot.value as Map);
      data.forEach((key, value) {
        if (value is Map) { // Check if value is a map before casting
    ...
    textures.sort((a, b) => (a['name'] ?? '').toLowerCase().compareTo((b['name'] ?? '').toLowerCase()));
    return textures;
  }
      ...
      body: FutureBuilder<List<Map<String, dynamic>>>(
        ...
        builder: (context, snapshot) {
          ...
          if (snapshot.hasError) {
            return Center(child: Text("Error loading textures: ${snapshot.error}", style: TextStyle(color: Colors.red)));
          }
          // No Data State
          if (!snapshot.hasData || snapshot.data!.isEmpty) {
            return Center(child: Text("No ${widget.category.toLowerCase()} textures found.", style: TextStyle(color: Colors.grey.shade600)));
          }
  ...
  AppBar _buildAppBar() {
    return AppBar(
      title: Text("${widget.category} Wall Textures", style: GoogleFonts.poppins(fontWeight: FontWeight.bold, color: Colors.grey.shade800)),
  ...
  Widget _buildTexturesGrid(List<Map<String, dynamic>> textures) {
    return GridView.builder(
      ...
      itemBuilder: (context, index) {
        ...
        // Animate each card
        return _buildTextureCard(texture).animate().fadeIn(delay: (index * 50).ms, duration: 300.ms);
      },
    );
  }
  ...
  Widget _buildTextureCard(Map<String, dynamic> texture) {
    return GestureDetector( // Make card tappable
      onTap: () {
        Navigator.push(
          context,
          MaterialPageRoute(builder: (_) => TextureDetailPage(
            ...
            children: [
              ...
              Positioned.fill(
                child: CachedNetworkImage(
                  ...
                  errorWidget: (context, url, error) => const Center(child: Icon(Iconsax.gallery_slash, color: Colors.grey)),
                ),
              ),
                ...
                child: Container(
                  ...
                  decoration: BoxDecoration(
                    gradient: LinearGradient(
                        colors: [Colors.black.withValues(alpha: 0.8), Colors.transparent], // Darker gradient
                        begin: Alignment.bottomCenter,
                        end: Alignment.topCenter,
                        stops: const [0.0, 0.8] // Adjust stops for gradient spread
                    ),
                  ),
                  child: Text(
