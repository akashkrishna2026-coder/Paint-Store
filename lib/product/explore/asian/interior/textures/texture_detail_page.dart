...
class _TextureDetailPageState extends State<TextureDetailPage> {
  ...
  bool _isLoadingCombinations = true;
  ...
  Future<void> _fetchCombinationColorDetails() async {
    ...
    if (allColorCodes.isEmpty) {
      if (mounted) setState(() => _isLoadingCombinations = false);
      return;
    }
    ...
    try {
      ...
      if (categoriesSnapshot.exists && categoriesSnapshot.value is Map) {
        ...
        allColorCodes.forEach((code) => details.add({'code': code, 'name': 'Error Loading', 'hex': '#FF0000'}));
      }

      setState(() { _combinationColorDetails = details; _isLoadingCombinations = false; });
        ...
        children: [
          Container(
            ...
            decoration: BoxDecoration(
              ...
              boxShadow: isSelected ? [BoxShadow(color: Colors.deepOrange.withOpacity(0.2), blurRadius: 4, spreadRadius: 1)] : [],
            ),
            child: ClipRRect(
