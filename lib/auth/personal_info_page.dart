...
class _PersonalInfoPageState extends State<PersonalInfoPage> with TickerProviderStateMixin {
  ...
  Future<void> _useCurrentLocation() async {
    setState(() { _locLoading = true; _locStatus = 'Fetching location...'; });
        ...
        child: Container(
          ...
          child: Column(
            children: [
              ...
              const SizedBox(height: 20),
              SlideTransition(position: _slideAnimations[3], child: _buildTextField(controller: _phoneController, label: "Phone", icon: Icons.phone_outlined, keyboardType: TextInputType.phone)),
              const SizedBox(height: 20),
              SlideTransition(position: _slideAnimations[4], child: _buildTextField(controller: _addressController, label: "Address", icon: Icons.home_outlined, keyboardType: TextInputType.streetAddress)),
              ...
              SizedBox(
                ...
                child: ElevatedButton.icon(
                  ...
                  style: ElevatedButton.styleFrom(backgroundColor: Colors.deepOrange, foregroundColor: Colors.white, padding: const EdgeInsets.symmetric(vertical: 12), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(10))),
                ),
              ),
              const SizedBox(height: 6),
              Align(alignment: Alignment.centerLeft, child: Text(_locStatus, style: GoogleFonts.poppins(fontSize: 12, color: Colors.white))),
              const SizedBox(height: 20),
              SlideTransition(position: _slideAnimations[5], child: _buildTextField(controller: _pincodeController, label: "Pincode", icon: Icons.location_on_outlined, keyboardType: TextInputType.number)),
              const SizedBox(height: 20),
  ...
  Widget _buildTextField({required TextEditingController controller, required String label, required IconData icon, bool readOnly = false, TextInputType keyboardType = TextInputType.text}) {
    return TextField(
      ...
      decoration: InputDecoration(
        ...
        enabledBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(15), borderSide: BorderSide(color: Colors.white.withValues(alpha: 0.5), width: 1)),
      ),
    );
  }
